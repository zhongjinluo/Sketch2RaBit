...
import uuid
def get_id():
    uu_id = uuid.uuid4()   # 生成uuid
    suu_id = ''.join(str(uu_id).split('-'))[:10]   # 去掉uuid里的"-"符号
    return suu_id
...
def show_on_mesh(uv_path, input_obj, output_obj, is_quad=True, size=1024):
    ...
    if is_quad:
        # tranf_quad2tri('{}{}_pred.obj'.format(obj_root, name), obj_path, uvs_r, faces_r, face_uvs_r, norms_r, face_norms_r)
        tranf_quad2tri(input_obj, output_obj, uvs_r, faces_r, face_uvs_r, norms_r, face_norms_r)
    else:
    ...
    if not os.path.isdir('tmp'):
        os.mkdir('tmp')
    tmp_path = 'tmp/{}.bmp'.format(get_id())
    dilate_save_img(True, size, uv_path, tmp_path)
    
    # reflect texture on mesh
    reflect_result(uvPath=tmp_path, objPath=output_obj, mesh_path = output_obj)

    # interpolation
    interpolation4all(output_obj, output_obj)
